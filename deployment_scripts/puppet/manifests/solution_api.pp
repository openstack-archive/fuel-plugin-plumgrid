...
exec { 'Send all IPs to Solution API server':
  command => "/usr/bin/curl -H \'Content-Type: application/json\' -X PUT -d \'{\"director_ips\":\"$controller_ipaddresses\",\"edge_ips\":\"$compute_ipaddresses\",\"gateway_ips\":\"$gateway_ipaddresses\",\"virtual_ip\":\"$plumgrid_vip\",\"opsvm_ip\":\"$md_ip\"}\' http://$plumgrid_pkg_repo:8099/v1/zoNes/ZoNepK/ALLIPS" }

exec { 'Send Environment data to Solution API server':
  command => "/usr/bin/curl -H \"Content-Type: application/json\" -X PUT -d \'{\"solution_name\":\"Mirantis\",\"solution_version\":\"8.0\",\"pg_ons_version\":\"6.0.0.b14\",\"hypervisor\":\"Ubuntu\",\"hypervisor_version\":\"14.04\", \"kernel_version\":\"3.19.0-47-generic\",\"pg_cloudapex_version\":\"1.0\"}\' http://$plumgrid_pkg_repo:8099/v1/zones/zonepk/zoneInfo" }
