...
exec { 'Place nova sql connection url':
  command => "/bin/grep -q -F \"connection = mysql://nova:\" /etc/nova/nova.conf || /bin/sed -i '/#connection = <None>/a connection = mysql://nova:$nova_sql_password@32.0.0.21/nova?read_timeout=60' /etc/nova/nova.conf && service nova-api restart"
}
